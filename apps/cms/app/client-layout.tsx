...
import { FileText, LayoutDashboard, Menu, Send } from 'lucide-react';
...
function AuthGuard({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        if (!isPublicRoute && session && (session?.user as any)?.role !== 'ADMIN') {
...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <AuthGuard>
            {isPublicRoute ? (
                // For public routes, just show the content without sidebar
                <>{children}</>
            ) : (
                // For private routes, show the full dashboard layout
